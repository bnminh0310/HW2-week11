...
def visualizeObjective(lr_model, t1_vals, t2_vals, X, y):
    ...
    surf = ax.plot_surface(
        T1, T2, Z, rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0
    )
...
if __name__ == "__main__":
    ...
    init_theta = (
        np.matrix(np.ones((d + 1, 1))) * 10
    )  # note that we really should be initializing this to be near zero, but starting it near [10,10] works better to visualize gradient descent for this particular problem
    n_iter = 1500
    alpha = 0.01
